...
struct ExampleFunctionalTransformerInputOutputLocations final
    : k4FWCore::Transformer<edm4hep::MCParticleCollection(const std::vector<const edm4hep::MCParticleCollection*>&,
  ...
  ExampleFunctionalTransformerInputOutputLocations(const std::string& name, ISvcLocator* svcLoc)
      : Transformer(name, svcLoc,
                    ...
                    {
                        KeyValue("OutputCollection", "OutMCParticles"),
                    }) {}
  ...
  StatusCode initialize() override {
    ...
    const std::vector<std::string> outputsSingle = {"OutMCParticles"};
    ...
    if (!std::ranges::equal(outputLocations(), outputsSingle)) {
      throw std::runtime_error("Wrong default value for the output collection, expected {OutMCParticles}, got {" +
                               outputLocations()[0] + "}");
    }
  ...
  edm4hep::MCParticleCollection operator()(const std::vector<const edm4hep::MCParticleCollection*>&,
                                           const edm4hep::MCParticleCollection&,
...
DECLARE_COMPONENT(ExampleFunctionalTransformerInputOutputLocations)
