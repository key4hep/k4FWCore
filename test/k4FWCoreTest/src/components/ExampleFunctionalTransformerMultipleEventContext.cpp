...
struct ExampleFunctionalTransformerMultipleEventContext final
    : k4FWCore::MultiTransformer<std::tuple<Counter, Particle, LinkColl>(
          const EventContext&, const FloatColl&, const ParticleColl&, const SimTrackerHitColl&, const TrackerHitColl&,
          const TrackColl&, const RecoColl&, const LinkColl&)> {
  ExampleFunctionalTransformerMultipleEventContext(const std::string& name, ISvcLocator* svcLoc)
      : MultiTransformer(
            name, svcLoc,
...
DECLARE_COMPONENT(ExampleFunctionalTransformerMultipleEventContext)
