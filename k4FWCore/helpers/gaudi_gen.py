...
# Safe-write helper
# ---------------------------------------------------------------------------
def _safe_write(path: str, content: str, force: bool, label: str) -> bool:
    """
    Write *content* to *path*. If the file exists and *force* is False, refuse
    to overwrite and tell the user how to override. Returns True on success.
    """
    if os.path.exists(path) and not force:
        print(
            f"Refusing to overwrite existing {label} at {path!r}.\n"
            f"  Re-run with --force (or remove the file) if you really want to replace it.",
            file=sys.stderr,
        )
        return False
...
""",
    ...
    parser.add_argument(
        "class_name",
        help="Name of the C++ class to generate (e.g. MyProducer).",
    )
    parser.add_argument(
        "functional_type", nargs="?",
        choices=["consumer", "producer", "transformer", "filter"],
        help=(
            "Functional type. Omit to infer from --inputs / --outputs counts; "
            "supply 'filter' explicitly when a FilterPredicate is wanted."
        ),
    )
    parser.add_argument(
        "-i", "--inputs", nargs="*", default=[], metavar="TYPE:KEY",
        help=(
            "Input collections, one or more 'TYPE:KEY' specs. "
            "Example: 'edm4hep::MCParticleCollection:MCParticles'. "
    ...
    out_group.add_argument(
        ...
        help=(
            ...
            "Multiple outputs trigger MultiTransformer/tuple return generation. "
    ...
    out_group.add_argument(
        ...
        help=(
            ...
            "k4FWCore-only. Mutually exclusive with --outputs."
        ),
    )
    parser.add_argument(
        "-p", "--properties", nargs="*", default=[],
        metavar="TYPE:NAME:DEFAULT[:DESC]",
        help=(
            "Gaudi::Property<T> members to declare. Format "
            "'type:name:default[:description]'. Example "
            "'int:NumThreads:4:Worker thread count'."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Target framework: 'k4fwcore' (default, for Key4hep / FCC) or "
            "'gaudi' (vanilla Gaudi::Functional)."
        ),
    )
    parser.add_argument(
        "--use-class", dest="use_class", action="store_true", default=False,
        help="Generate 'class ... { public: ... }' instead of the default 'struct'.",
    )
    parser.add_argument(
        "-f", "--output-file", dest="output_file", default=None,
        help="Path for the generated .cpp file. Default: <ClassName>.cpp in the cwd.",
    )
    parser.add_argument(
        "--type-aliases", dest="type_aliases", action="store_true", default=False,
        help=(
            "Emit 'using XxxColl = ...;' aliases for input collection types "
            "and use them in the operator() signature."
        ),
    )
    parser.add_argument(
        "--private-properties", dest="private_properties",
        action="store_true", default=False,
        help="Place Gaudi::Property<T> members under a 'private:' access label.",
    )
    parser.add_argument(
        "--all-keyvalues", dest="all_keyvalues",
        action="store_true", default=False,
        help=(
            "Treat every --inputs entry as a runtime KeyValues vector "
            "(std::vector<const T*>&) instead of a single KeyValue."
        ),
    )
    parser.add_argument(
        "--keyvalues-inputs", dest="keyvalues_inputs", nargs="*", default=None,
        metavar="KEY[:LABEL]",
        help=(
            "Per-input override: turn the named --inputs KEY(s) into KeyValues "
            "vector inputs while leaving the others as scalars."
        ),
    )
    parser.add_argument(
        "--runtime-inputs", dest="runtime_inputs", nargs="*", default=None,
        metavar="TYPE:KEY:DEF1[,DEF2,...]",
        help=(
            "Declare runtime (variable-length) inputs with default location "
            "names. Example "
            "'edm4hep::MCParticleCollection:Inputs:MCParticles0,MCParticles1'. "
            "k4FWCore-only."
        ),
    )
    parser.add_argument(
        "--event-context", dest="event_context",
        action="store_true", default=False,
        help=(
            "Add 'const EventContext&' as the first operator() argument and "
            "scaffold a finalize() override with mutable bookkeeping members."
        ),
    )
    parser.add_argument(
        "--cmake", action="store_true", default=False,
        help="Also emit a CMakeLists.txt next to the source (cwd).",
    )
    parser.add_argument(
        "--force", action="store_true", default=False,
        help=(
            "Allow overwriting existing files. Without this flag, the script "
            "refuses to clobber an existing .cpp or CMakeLists.txt."
        ),
    )
    return parser
...
def main() -> None:
    ...
    cpp_ok = _safe_write(spec.output_file, cpp_source, args.force, label="C++ source")
    cmake_ok = True
    if cmake_source is not None:
        cmake_ok = _safe_write("CMakeLists.txt", cmake_source, args.force, label="CMakeLists.txt")

    if not (cpp_ok and cmake_ok):
        sys.exit(1)
