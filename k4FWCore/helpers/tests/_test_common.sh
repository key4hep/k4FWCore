...
#   run_cmake_build ClassName [gaudiGen.py args...]
...
if command -v gaudiGen.py &>/dev/null; then
    GENERATOR="$(command -v gaudiGen.py)"
else
    GENERATOR="${SCRIPT_DIR}/../gaudiGen.py"
fi

if [[ ! -f "${GENERATOR}" ]]; then
    echo "ERROR: gaudiGen.py not found (tried PATH and ${GENERATOR})" >&2
    exit 1
fi
...
# run_cmake_build <ClassName> [gaudiGen.py args...]
#   1. Generates <ClassName>.cpp + CMakeLists.txt via gaudiGen.py --cmake
